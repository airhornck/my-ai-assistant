...
**explicit_content_request（关键）**：用户是否**明确要求生成具体内容**（文案、文章、脚本、推广方案等）？
- true：**只要用户说了「帮我生成」「帮我写」「帮我做」「制定」「创建」「生成」等动作词，后面跟着具体内容类型（文案、脚本、文章、方案、推广策略等），一律判为true**。
  - 典型例子：「帮我生成小红书文案」「帮我写一个B站脚本」「制定一个推广策略」「生成一篇营销文章」「帮我做一套推广方案」「帮我写抖音视频脚本」「帮我生成多平台的推广文案」
  - 注意：即使只说了「帮我推广」而没有说具体内容类型，也应判为true，因为「推广」隐含了生成推广内容的需求。
- false：用户只是陈述话题、目标人群、推广意向，**未明确要求产出具体内容**。
  - 典型例子：「推广华为手机，年龄18-35」→ false；「我是卖女装的」→ false；「想提升账号流量」→ false
  - 区分要点：看用户是否要求"生成/写/做"某个具体内容
...
EXPLICIT_CONTENT_PHRASES = (
    ...
    "推广文案", "推广脚本", "推广文章", "营销文案", "营销脚本",  # 推广类内容
    "多平台", "多个平台", "三个平台", "各平台",  # 多平台
    "帮我推广", "帮我制定", "制定一个", "帮我创建",  # 动作词
)
...
class InputProcessor:
    ...
    async def process(
        ...
        # 结构化请求或自由讨论中提到具体平台/内容类型时，也视为明确要生成
        if intent in (INTENT_STRUCTURED_REQUEST, INTENT_FREE_DISCUSSION):
            platform_keywords = ("小红书", "抖音", "B站", "b站", "微博", "知乎", "快手", "视频号", "文案", "脚本", "推广")
            if any(kw in raw for kw in platform_keywords):
                base["explicit_content_request"] = True
                logger.debug("explicit_content_request=true (自由讨论+平台关键词触发)")
        
        logger.info("IntentProcessor输出: intent=%s, explicit_content_request=%s", intent, base.get("explicit_content_request"))
