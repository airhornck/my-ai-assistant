...
**终止点**：当本轮已达成目标（已生成并评估、内容完整、用户无继续迭代意愿）时，只输出 1 条简短收尾，例如「本次内容已就绪，有新需求随时说。」此时**建议用户生成 Word 报告留存**，如「可以把这份方案下载保存，我会帮你生成一份 Word 版报告，方便你留存参考。」

**生成报告的时机**：当内容已完成（generate 步骤已执行且评估通过），应该建议用户生成报告留存，例如：
- 「这份诊断报告很详细吧？我可以帮你生成一份 Word 版报告，方便你留存和分享。」
- 「方案已经生成完毕，需要我帮你导出 Word 版吗？以后随时可以查看。」

**STEP 仅用于系统**：若需要系统执行下一步（generate 或 analyze 或 word_report），在回复的**最后单独另起一行**只写：STEP: generate 或 STEP: analyze 或 STEP: word_report。该行仅供系统识别，不会展示给用户，所以必须单独一行、不要和正文写在同一行。"""
...
def _parse_step_from_response(text: str) -> tuple[str, str]:
    ...
    for line in lines:
        ...
        m = re.search(r"\s*STEP:\s*(generate|analyze|word_report)\s*$", s, re.IGNORECASE)
        if m:
            step_name = m.group(1).lower()
            s = s[: m.start()].strip()
        rest_lines.append(s)
    rest = "\n".join(rest_lines).strip()
    if step_name not in ("generate", "analyze", "word_report"):
